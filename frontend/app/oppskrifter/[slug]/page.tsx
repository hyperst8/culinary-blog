...
import { InfoCircle, AlertCircle } from "@untitledui/icons";
    ...
    <div className="container py-8 md:py-12 max-w-5xl">
      ...
      <div className="bg-surface rounded-2xl p-6 md:p-8 shadow-sm border border-surface-muted mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex-1">
          <h1 className="text-3xl md:text-4xl font-bold text-text-tertiary mb-3">
            {recipe.title}
          </h1>
        ...
        <div className="flex flex-wrap items-center gap-3 shrink-0 bg-surface-muted p-4 rounded-xl border border-brand-50">
          ...
          <Badge type="category" icon={false}>
            {recipe.mealType}
          </Badge>
        </div>
      </div>
              ...
              {recipe.ingredients?.map((ing, i) => (
                <li key={i} className="flex items-start gap-3 ml-2">
                  <span className="text-text-secondary">
                    {ing.amount && (
                      <span className="font-semibold text-text-tertiary mr-1">
                        {ing.amount}
                      </span>
                    )}
                    {ing.name}
          ...
          <div className="flex flex-col gap-8 mb-12">
            {recipe.instructions?.map((step, i) => (
              <div key={i} className="flex gap-4">
                ...
                <div>
                  <h3 className="text-lg font-bold text-text-tertiary mb-2">
                    {step.title}
                  </h3>
            ...
            {recipe.tips?.map((tip, i) => (
              ...
              >
                <h4 className="font-bold mb-2 flex items-center gap-2">
                  {tip.type === "equipment" ? (
                    <InfoCircle className="w-5 h-5 text-brand-600" />
                  ) : (
                ...
                <p
                  className={`text-sm leading-relaxed ${tip.type === "equipment" ? "text-brand-800" : "text-red-800"}`}
                >
                  {tip.description}
                </p>
              </div>
